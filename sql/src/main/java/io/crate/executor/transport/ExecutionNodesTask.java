...
public class ExecutionNodesTask extends JobTask {
    ...
    private final int directResultNode;
    ...
    protected ExecutionNodesTask(UUID jobId,
        ...
        directResultNode = getDirectResultNode(executionNodes);

        RowDownstream rowDownstream = new QueryResultRowDownstream(result, jobId, jobContextService);
        ...
        if (directResultNode == JobRequest.NO_DIRECT_RETURN) {
    ...
    private int getDirectResultNode(ExecutionNode[] executionNodes) {
        int idx = 0;
        for (ExecutionNode executionNode : executionNodes) {
            if (executionNode.downstreamExecutionNodeId() == ExecutionNode.NO_EXECUTION_NODE) {
                return idx;
            }
            idx++;
        }
        return JobRequest.NO_DIRECT_RETURN;
    }
    ...
    public void start() {
        ...
        for (Map.Entry<String, Collection<ExecutionNode>> entry : nodesByServer.entrySet()) {
            ...
            JobRequest request = new JobRequest(jobId(), executionNodes, directResultNode);
            if (directResultNode == JobRequest.NO_DIRECT_RETURN) {
